...
const isExplicitAwait = (value) => (isThenable(value) && value[isAwaitPromise] === true);
...
const resolveAwaitedValues = (value) => {
    ...
    if (Array.isArray(value)) {
        ...
        if (!hasAwait)
            return result;
        const awaited = result.map((item, index) => isExplicitAwait(item)
    ...
    if (isPlainObject(value)) {
        ...
        for (const key of Object.keys(value)) {
            ...
            if (isExplicitAwait(resolved)) {
                promises.push(Promise.resolve(resolved).then((resolvedValue) => {
...
export default function createParser(env) {
    return function parse(program, leafDeserializer) {
