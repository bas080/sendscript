...
import Sendscript from '../src/index.js'

const module = {
  add: (a: number, b: number) => a + b,
  identity: (x: any) => x,
  concat: (a: any, b: any) => a.concat(b),
  toArray: (...array: any[]) => array,
  always: (x: any) => () => x,
  multiply3: (a: number) => (b: number) => (c: number) => a * b * c,
  map: (fn: Function) => (array: any[]) => array.map(fn),
  filter: (pred: Function) => (array: any[]) => array.filter(pred),
  hello: 'world',
  noop: () => {},
  resolve: (x: any) => Promise.resolve(x),
  asyncFn: async () => 'my-async-function',
  instanceOf: (x: any, t: any) => x instanceof t,
  asyncAdd: async (a: number, b: number) => a + b,
  aPromise: Promise.resolve(42),
  delayedIdentity: async (x: any) => x,
  nullProto: () => {
...
const run = (program: any) => parse(stringify(program))
...
test('should evaluate basic expressions correctly', async (t) => {
  ...
  t.test('promise resolution', async (t) => {
    ...
    t.strictSame(run(instanceOf(resolve(asyncFn), PromiseRef)), true)
    t.strictSame(
      await run(instanceOf(await resolve(asyncFn), FunctionRef)), true
    )
    t.strictSame(
